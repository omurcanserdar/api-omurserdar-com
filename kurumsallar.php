...
ob_start();
 ...
 $title="Kurumsal Üyeler Listesi ve Bölgesel Arama Sayfası  | API";
 $desc="kurumsal üyelerin listelendiği, bölgesel olarak aramaların gerçekleştirilebileceği sayfa";
 include "header.php";
 
 //tanımlayabildiğim enteresanlıklara karşı önlemler
if(isset($_GET["ilce"]) and !isset($_GET["il"])){ 
       //"istekte ilçe var il yok";
...
<?
if(isset($_GET["ilce"]) or (isset($_GET["il"]) and ($_GET["il"]>0 and $_GET["il"]<82))){
...
<!-- İl ilçe AJAX-->
 ...
 $(function(){
     ...
     $("#btnara").prop("disabled",true);
     
     if($("#illerf2 select").val()==0){
         $("#ilcelerf2 select").prop("disabled",true);
          $("#btnara").prop("disabled",true);
     }
     else{
         $("#ilcelerf2 select").removeAttr('disabled');
          $("#btnara").removeAttr('disabled');
     }
...
<button type="submit" id="btnara" class="btn btn-outline-primary btn-block"><i class="fas fa-search"></i> ARA </button>
...
<div class="col-md-9">
       ...
       <?php 
        ...
        if(!isset($_GET["il"]) and !isset($_GET["ilce"])){
        ...
        $durum=""; $adres=" ";
        if(isset($_GET["il"]) and ($_GET["il"]>0 and $_GET["il"]<82)){
            $ilid=$_GET["il"];
            $durum.="il.id=$ilid";
  ...
  <div class="col-md-4"> 
    <div class="card text-white mr-1 mt-1 bg-<?=$renk?>">
     <div class="card-header float-left" id="chka<?=$tumkurumsalcek["id"]?>"><small><?=$tumkurumsalcek["ad"]?></small> <?if($tumkurumsalcek["acikMi"]==0){
            echo '<small class="float-right"><i class="fas fa-lock"></i> KAPALI - Sipariş Verilemez</small>';
        }
        else{
            echo '<small class="float-right"><i class="fas fa-lock-open"></i> AÇIK - Sipariş Verilebilir</small>';
        }
        ?></div>
      <div class="card-body mx-auto">

          <small><i class="fas fa-map-marker-alt"></i> <?=$tumkurumsalcek["ilce_adi"]." / ".$tumkurumsalcek["il_adi"]?> </small>
          
          <div align="center" class="mt-1" data-id="<?=$tumkurumsalcek["kullaniciadi"]?>" id="qrcode<?=$qrsayac?>"></div>
